...
with open("example_data/blast_results.xml") as handle, \
    ...
    for record in NCBIXML.parse(handle):
        ...
        for alignment in record.alignments:
            ...
            output.write(
                f"{query_id}\t{query_len}\t{hit_id}\t{hit_len}\t"
                f"{identity:.6f}\t{bit_score:.2f}\t{e_value:.2e}\t"
                f"{query_start}\t{query_end}\t{hit_start}\t{hit_end}\t"
                f"{query_coverage:.2f}\t{subject_coverage:.2f}\n"
            )

print("Done! Results saved to output_coverage.tsv")
